...
int main(){
	...
	// We want to use a base pointer because they can point to all base objects as well as all derived objects.  
	// In this case, it is even more useful because we can use an array of object pointers, which will let us use loops.
	...
	// When you run this, it only calls the base class print functions
	// The overridden print functions in the derived classes are ignored because the pointers of base type
	// If we want to get the correct functions, but also use base pointers, we need a change - see next
